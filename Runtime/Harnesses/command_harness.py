...
COMMAND_HARNESS_IDS = frozenset({"test", "performance", "scm"})
...
def run_command_harness(
    *,
    ...
    validation_scope: str = "targeted",
    validation_reason: str = "",
) -> dict:
    if harness_id not in COMMAND_HARNESS_IDS:
        raise ValueError(f"unsupported command harness id: {harness_id}")
    if validation_session is not None:
        if harness_id != "test":
            raise ValueError("validation session applies only to the test harness")
        return validation_session.run(
            command=command,
            cwd=cwd,
    ...
    return {
        ...
        "exit_code": result["result"].returncode,
