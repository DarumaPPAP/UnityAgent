...
def _tool_identity(request: dict, model: str, provider: str) -> dict:
    ...
    return {"provider": provider, "model": model, "model_revision": str(identity.get("model_revision") or model), "tool_manifest_hash": str(identity.get("tool_manifest_hash") or "runtime-codex-v1"), "executor_profile": identity.get("executor_profile"), "execution_mode": identity.get("execution_mode")}
...
def execute(request: dict[str, Any], output: Path, *, command_prefix: list[str], timeout_seconds: float, reasoning_effort: str) -> dict:
    ...
    guard = evaluate_mutation_scope(work_kind=str(execution.get("work_kind") or "analysis"), changed_paths=changed, allowed_paths=list(scope.get("allowed_paths", []) or []), prohibited_paths=list(scope.get("prohibited_paths", []) or []))
    ...
    metrics = {"changed_paths": changed, "runtime": {"timed_out": process.timed_out, "cancelled": process.cancelled, "duration_seconds": process.duration_seconds, "process_tree_cleanup": process.process_tree_cleanup, "remaining_processes": process.remaining_processes, "event_count": process.event_count}}
