"""Historical replay normalization for legacy layered-memory records."""
...
MEMORY_LAYERS = {
    ...
    "L3_reusable_candidate": "reusable_candidate",
}
...
def _raw_id(value: str) -> str:
    ...
    if not normalized.startswith("Evidence/raw/"):
        raise LegacyReplayError("legacy_raw_ref_invalid", f"raw ref is outside Evidence/raw: {value}")
...
def normalize_legacy_layered_records(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(by_id) != len(records):
        raise LegacyReplayError("legacy_memory_invalid", "legacy memory ids must be present and unique")
    ...
    def evidence_ancestry(memory_id: str) -> set[str]:
        ...
        if memory_id in visiting:
            raise LegacyReplayError("legacy_memory_cycle", f"legacy memory cycle at {memory_id}")
        visiting.add(memory_id)
        record = by_id.get(memory_id)
        if record is None:
            raise LegacyReplayError("legacy_memory_reference_missing", f"legacy memory reference missing: {memory_id}")
        ...
        if layer == L0:
            ...
            for ref in record.get("raw_refs", []):
                ...
                if raw_id not in by_id or by_id[raw_id].get("layer") != L0:
                    raise LegacyReplayError("legacy_memory_reference_missing", f"L1 raw evidence metadata missing: {raw_id}")
                result.add(raw_id)
            ...
            raise LegacyReplayError("legacy_memory_layer_unknown", f"unsupported legacy layer: {layer}")
