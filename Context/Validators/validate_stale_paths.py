...
def validate_added_lines(root: Path = Path("."), base_ref: str | None = None) -> list[str]:
    ...
    try:
        diff = subprocess.check_output(["git", "diff", "--unified=0", base_ref, "HEAD", "--"], cwd=root, text=True, encoding="utf-8")
    except Exception:
        return errors
    current = ""
    for line in diff.splitlines():
        ...
        if LEGACY_MARKER in added and re.search(r"(write_text|write_bytes|open\s*\(|unlink|rename|replace)", added):
