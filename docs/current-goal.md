...
MyResourceCenterとUnityAgentを、常時稼働サーバー・Embedding・Qdrant・自動同期に依存せず、必要時の一度のSource確認で生成したローカルReference Snapshotを中心に運用できる状態にする。
...
- 個人運用での常時稼働Knowledge/RAG Service
- Embedding API、Qdrant、Vector DB、常時同期worker
- 参照文を命令として実行すること
