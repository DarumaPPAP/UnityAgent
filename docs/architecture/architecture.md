...
- Legacy v1: `Runtime/Contracts/entry-request.schema.yaml` remains unchanged for validation; its caller supplied Context identity is rejected by `UnityAgentControlPlane.execute`. Migrate callers to v2 by removing Entry-owned Context / Route / Capability / Handoff values and supplying explicit task intent and seven semantic Task Fingerprint dimensions. Orchestration selects the Route, generates provider-independent CapabilityRequests from the canonical routing catalog, and hands the Context Assembly-generated identity to Runtime. The Project access dimension comes from observed binding and Policy, never from Entry.
