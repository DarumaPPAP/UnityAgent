...
class EvidenceStore:
    ...
    def append(self, record: dict[str, Any]) -> bool:
        required = {
            "schema_version", "evidence_id", "run_id", "step_id", "source_type",
            "source_ref", "timestamp", "hash", "producer", "verification_status",
            "provenance", "definition_fingerprint",
        }
        missing = sorted(required - set(record))
        if missing:
            raise PersistenceError("contract_missing_field", f"EvidenceRecord missing required fields: {missing}")
        if record.get("schema_version") not in {"1.0", "1.1"}:
            raise PersistenceError("invalid_evidence_record", "unsupported EvidenceRecord schema_version")
        ...
        if created:
            append_jsonl(self.layout.evidence_events(), {
