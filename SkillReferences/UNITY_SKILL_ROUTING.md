...
| `READY` | 直接実装可能か、設計が必要か、調査が必要か | Implement / Plan / Incident | Execution Contract |
| `PLANNING` | Spec、Plan、Tasksのどこまで必要か | `unity-specify` / `unity-plan` / `unity-tasks` | 承認可能な計画 |
...
| `DOMAIN_VALIDATION` | Rendering、Shader、Variant、AOT等 | Domain specialist | Domain evidence |
| `VISUAL_OR_RUNTIME_VALIDATION` | Screenshot、Player、実機、Profiler | Evidence Skill | Runtime evidence |
| `EVIDENCE_REVIEW` | Goalと証拠が一致するか | `unity-review` | Accept / Rework / Revert / Inconclusive |
...
| Visual mismatch | `DOMAIN_VALIDATION` | `unity-rendering` / Shader specialist | Before / After、Camera、Render event、Depth / Blend | Compile成功だけの承認 |
...
| Missing tool / unavailable device | `CONTEXT_REQUIRED`または`BLOCKED` | Supervisor | 未取得証拠 | 実行済みと推測 |
...
1. **User-requested outcome** — 説明、調査、実装、レビュー、計測
...
6. **Verification requirement** — 性能主張はEvidence Skillを必須にする
7. **Human boundary** — 破壊的契約やMergeをAgentだけで決定しない
